...
//            request.setData(JSON.parse(jsonStr.toString()));
            request.setData(codec.decodeBody(channel, request, dataBytes));
            out.add(request);
...
//                response.setData(JSON.parse(jsonStr.toString()));
                response.setData(codec.decodeBody(channel, response, dataBytes));
                System.out.println("接收正常响应：" + response.getData());
            } else {
//                response.setErrorMsg(JSON.parseObject(jsonStr.toString(), String.class));
                response.setErrorMsg((String) codec.decodeBody(channel, response, dataBytes));
